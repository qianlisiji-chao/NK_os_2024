...
static int
get_pid(void) {
    static_assert(MAX_PID > MAX_PROCESS);//确保足够大
    struct proc_struct *proc;
    list_entry_t *list = &proc_list, *le;
    static int next_safe = MAX_PID, last_pid = MAX_PID;//last_pid从MAX_PID开始使用
    if (++ last_pid >= MAX_PID) {
        last_pid = 1;
        goto inside;
    }
    //last_pid比next_safe大，寻找一个安全的PID
    ...
    repeat:
        ...
        while ((le = list_next(le)) != list) {
            ...
            //如果发现已有进程的 PID 与当前 last_pid 冲突：将 last_pid 自增并重新检查，直到找到一个未被占用的 PID。
            if (proc->pid == last_pid) {
                if (++ last_pid >= next_safe) {
                    //如果超过最大值 MAX_PID，从 1 开始重新分配（避免溢出）
                    if (last_pid >= MAX_PID) {
                        last_pid = 1;
                    }
                    next_safe = MAX_PID;
            ...
            //如果发现进程的 PID 大于当前 last_pid，更新 next_safe
